...
class DataFoursquare(object):
    def __init__(self, trace_min=10, global_visit=10, hour_gap=72, min_gap=10, session_min=2, session_max=10,
        ...
        tmp_path = "../data/"
        self.data_path = os.path.join(tmp_path, "dataset_TSMC2014_NYC.csv")
        self.SAVE_PATH = tmp_path
        self.save_name = 'foursquare_checkin'
    ...
    def filter_users_by_length(self):
        ...
        for u in pick3:
            ...
            for i, record in enumerate(info):
                poi, tmd = record
                try:
                    tid = int(time.mktime(time.strptime(tmd, "%a %b %d %X %z %Y")))
    ...
    def load_venues(self):
        with open(self.data_path) as fid:
            ...
            for i in range(len(lines)):
                if i > 0:
                    line = lines[i]
                    uid, pid, _, _, lon, lat, _, tim = line.strip("\n").split(',')
    ...
    @staticmethod
    def tid_list_48(tmd):
        tm = time.strptime(tmd, "%a %b %d %X %z %Y")
